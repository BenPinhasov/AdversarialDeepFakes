...
# class new_SiameseTuning(FeatureExtractor):
#     def __init__(self, feat_ext: FeatureExtractor, num_feat: int, lastonly: bool = True):
#         super(SiameseTuning, self).__init__()
#         self.feat_ext = feat_ext()
#         if not hasattr(self.feat_ext, 'features'):
...
#         self.classifier = nn.Sequential(
#             nn.BatchNorm1d(num_features=num_feat),
#             nn.Linear(in_features=num_feat, out_features=2),
#         )
#
#     def features(self, x):
#         x = self.feat_ext.features(x)
...
# class new_EfficientNetB4ST(new_SiameseTuning):
#     def __init__(self):
#         super(new_EfficientNetB4ST, self).__init__(feat_ext=EfficientNetB4, num_feat=1792, lastonly=True)
